...
- **GND 覆铜待客户端操作：**`pcb_PrimitivePour.create()` 在本机 3.2.203 上六种入参全被拒，必须在 GUI 里"放置 → 覆铜 → 网络 GND"分别铺顶层和底层后保存，详见 [E16 记录](hardware/pcb-e16-usb-right-mid.md#待用户操作gnd-覆铜)。
...
- 下一步按[E16 右侧中部 USB 布局](hardware/pcb-e16-usb-right-mid.md)继续：**先做 GND 覆铜**（需客户端操作）并复核地回流与新增间距错误，再核对 NFC 预留区的禁布约束与馈线方案，随后刷新审查图、把外壳样件推进到含卡扣与公差、与冻结板框一致的版本，最后才是制造导出与下单。到货后仍需验证 FPC、刷新/功耗、电池与 NTC。
