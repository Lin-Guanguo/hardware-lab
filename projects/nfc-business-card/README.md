...
status: pcb_routed_drc_clean_cad_sample_v2
---
...
- **CAD 进展（V5，当前）：**[E16 外壳样件 V5](enclosure/nfc-card-e16-enclosure-v5.FCStd) 跟随 L 形板框（左下角 33.5 × 15 mm 电池挖空）更新：电芯改坐在下壳底板上、不再压在 PCB 上，整机厚度从 5.0 降到 **4.5 mm**（0.4 底盖 + 0.3 板下间隙 + 0.8 PCB + 2.5 顶腔 + 0.5 上盖）。V3 的两块打印板也按 L 形生成，电池袋上下都缺板、加强筋悬空；V4 把打印板改成整卡轮廓减 USB 缺口，电池袋重新封闭（底板 201 mm³ + 上盖 251.25 mm³ 写进回归检查）。几何检查通过（上下壳各一实体、无壳体/参考件相交、连接器零接触）。V5 另加两个齐平键帽（Ø4.2 圆片骑在 Ø4.6 孔里、行程柱压在开关上，总高 1.45 mm）并按 SKQGABE010 的 1.5 mm 本体/0.25 mm 行程校核；加强筋 #1 西端收到键帽东侧 0.8 mm 外，避免与按压行程干涉。上盖保留三条加强筋防止 0.5 mm 薄板塌陷；屏幕窗口按 GDEH0154E01 图纸压在有效区上、屏幕架空 1.51 mm 用垫片压紧。电芯最大外形、键帽、屏幕贴合与壁厚公差仍需实物验证。
...
| 交互 | 两键：一级栏目轮换 / 二级选项轮换，二级选到即生效；第三键已取消（原备用输入保留在主控） |
