...
- **连通性**：0 断网、0 死端。
- **原生 DRC 返回 `false`，但改动前也是 `false`**（门槛文件记录改动前就有 24 项 `Board Outline to SMD Pad` 槽边告警），所以这个布尔值对本次改动没有分辨力。本次改动的间距结论以上面那条独立 diff 校验为准。**注意 DRC API 只返回布尔值，明细在 UI 面板里，没有读取接口**——这是本仓库的一条验证空白。
...
6. **补原生 DRC 明细的可读路径**：DRC API 只返回布尔值，明细在 UI 面板里。目前只能靠"改动前后布尔值都是 false + 独立 diff 校验"来判定没有新增错误，这条链偏弱。
