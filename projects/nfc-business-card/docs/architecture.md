...
## 4.5 固件接口要求（2026-09-23 由硬件定案）

三条要求是从已定稿的硬件里读出来的，写固件时必须对上；原始依据在[E16 设计记录](../archive/pre-r1/hardware/pcb-e16-usb-right-mid.md)。

1. **充电电流**：BQ25186 的充电电流完全由 I2C 设定（5–1000 mA），没有硬件电阻。`ICHG_CTRL`(0x4) 复位值 `0x05` = `CHG_DIS 0` + `ICHGCODE 5`，即**上电默认只充 10 mA**。对 40–60 mAh 电芯应在初始化时把 `ICHGCODE` 写到 **15–20（20–25 mA，约 0.5C）**；`ITERM` 默认 10% of ICHG 适合这么小的电芯。手册 6.5.1.5/6.5.1.6。
2. **NTC / TS 引脚**：`TS/MR`(U2 pin 6) 只连到板上的 `NTC` 焊盘。电芯自带热敏就把 NTC 引线焊到 `NTC`–`BN` 并正常读温度；**没有热敏时必须在初始化里清 `TS_EN`**（6.3.15.1：只关掉 TS 的充电控制，故障仍然上报），否则 TS 电压落在有效窗口之外会挂起快充与涓流；也可以临时在 `NTC`–`BN` 之间焊一个 10 kΩ 假负载。
3. **按键映射**：SW1（U1 pin 3 / P1.10）= 键 1，SW3（U1 pin 5 / P1.12）= 键 2；U1 pin 4（P1.11，`KEY_NEXT_N`，带上拉）接第三颗开关 SW2 (46.3, 8.80)，功能待定、暂作备用输入。行为见下面第 5 节。

屏幕侧仍需固件确认的量（BUSY 极性、刷新时序、峰值电流）见[接口评估](gdeh0154e01-evaluation.md)。

## 5. 二级菜单与切换一致性

2026-09-21 确定基本交互：**两键完成全部基本操作。** 2026-09-23 的取舍是：先试“三颗重排成一个紧凑竖列”（中心距只有 5.9 mm、本体只剩 0.7 mm 缝，太挤），随后定案**三角形三键（方案 B）**——SW1/SW3 保持原一列（中心距 11.80 mm），第三颗 SW2 放在按键列右侧空地 (46.3, 8.80)，与另外两颗各约 8.4 mm，外壳 V7 开三个键孔。第三颗接在原本空置的带上拉输入 `KEY_NEXT_N` 上，功能待定，因此**交互仍只需要键 1 / 键 2**；以后要用它（例如“上一项 / 下一项”或快捷键）只改固件即可，板子无需改版。以下“键 1 / 键 2”是逻辑角色，与 SW1/SW3、GPIO 及既有网络名的对应关系见[E16 设计记录](../archive/pre-r1/hardware/pcb-e16-usb-right-mid.md#三键已实施2026-09-23方案-b)。

| 输入 | 硬件 | 行为 |
| --- | --- | --- |
| 键 1 | SW1（y=3.30）→ U1 pin 3 / P1.10，`KEY_PREV_N` | 循环切换一级栏目，只导航，不改变已生效设置 |
| 键 2 | SW3（y=15.10）→ U1 pin 5 / P1.12，`KEY_OK_N` | 循环切换当前栏目的二级选项，选到即自动应用，无需单独确认 |
| 键 3（备用） | SW2 (46.3, 8.80) → U1 pin 4 / P1.11，`KEY_NEXT_N` 带上拉 | 功能待定；固件可定义为“上一项 / 下一项”或快捷键，硬件无需改动 |
