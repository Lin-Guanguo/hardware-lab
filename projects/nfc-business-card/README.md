...
- 当前 EDA：[NFC-Business-Card-84x52-E14-Battery-Layout.eprj2](../../eda/NFC-Business-Card-84x52-E14-Battery-Layout.eprj2) 内的 **`Board1_2` / E16 Right-Mid USB Study**（2026-09-23 用 `dmt_Board.createBoard` 与 `Schematic1` 关联）：**56 个元件、242 个焊盘、783 段铜线、162 个过孔、两层 GND 覆铜**；L 形板框（84 × 52，左下 33.5 × 15 mm 电池挖空 + 右边缘 USB 缺口）。原生 DRC 保存/关闭/重开后 **连接 0**，只剩 24 项 `Board Outline to SMD Pad`：12 个 J1 信号焊盘离板边 0.2007 mm（板厂允许 ≥0.2 mm），板框侧与封装槽线侧各判一次。原理图与 PCB **逐引脚 234 项、0 处网名差异**。[E15 底边 USB 版](hardware/pcb-e15-clean-layout.md)与 `Board1/E6` 保留作回退。
