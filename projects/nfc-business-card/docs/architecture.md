...
三条要求是从已定稿的硬件里读出来的，写固件时必须对上；原始依据在[E16 设计记录](../hardware/pcb-e16-usb-right-mid.md)。
