...
**视觉检查（2026-09-23）**：E16 样件用纯 Python 从 STL 渲染出着色等轴测图和正视顶视图（`artifacts/e16-cad-preview/e16-enclosure-v5-{iso,top}.png`，脚本 [render-stl-iso.py](../scripts/render-stl-iso.py)，不依赖 FreeCAD GUI）。图中确认：下壳是整片底板 + L 形台阶，上壳正面完整覆盖电池袋（不再是通孔），屏窗、两个 Ø4.6 mm 键孔（V5 键帽在两孔里齐平）和右边缘 USB 缺口位置与设计一致；上下壳各为单一实体、键帽各为单一实体，没有悬空面。几何检查与视觉检查现已分别完成。
...
**E16 V5 的装配顺序（2026-09-23 定，待样件验证）**

打印件共 4 件：下壳、上壳、两个键帽（[键帽 STL](nfc-card-e16-keycaps-v5.stl)，两件同料一次打）。打印时下壳/上壳平放（大平面朝下）、键帽立着打（轴竖直，Ø4.2 圆片朝上），避免支撑落在键孔的配合面上。

1. 去毛刺后先试装键帽：Ø4.2 圆片应能在 Ø4.6 孔里自由滑动，两个键帽顶面与上壳齐平（z=4.5），不装开关时能靠自重落到底、按键柱下端到 z≈3.05。
2. 从壳内侧把两个键帽放进键孔，键帽柱对准 SW1（y=3.30）/ SW3（y=15.10）。
3. 电芯放进下壳电池袋（33.5 × 15 mm，底板 0.4 mm）：底板上贴 0.5 mm 双面胶/泡棉定位，电芯顶面与上盖内表面留 0.6 mm；引线走向待实物确认。
4. PCB 放进下壳台阶：J1 对准右边缘缺口（缺口 y 11.38–20.62），先看 USB 插头能否顺畅插入；确认后沿 1.0 mm 台阶环带点胶。
5. 屏幕背面贴 1.5 mm 泡棉（压缩量约 0.5 mm），背面朝上放进上壳，靠上壳凸台压住模块边框。
6. 合上上壳压合固化：上壳台阶同时压在 PCB 顶面，屏幕被泡棉顶到凸台上，J1 在缺口处不外露缝隙。
7. 装配后检查：两个键帽顶部与上壳齐平、按下 0.25 mm 有明确手感；屏幕窗口对正；USB 插头能插到底；整机厚度卡尺量 ≈4.5 mm。
