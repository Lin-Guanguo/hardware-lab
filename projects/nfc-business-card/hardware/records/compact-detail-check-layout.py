...
plan=json.loads((root/'plan.json').read_text());a=read_result('saved');b=read_result('reopened');ra,rb=rows(a['source']),rows(b['source'])
source_normalizations=[]
for k in set(ra)&set(rb):
 if k[0]=='PAD' and ra[k]['layerId']==1 and ra[k]['hole'] is None and ra[k]['bottomSolderExpansion'] is None and rb[k]['bottomSolderExpansion']==0:
  source_normalizations.append('Client materialized unused bottom mask expansion on top-only SMD pads')
  ra[k]['bottomSolderExpansion']=0
 if k[0]=='COMPONENT':
...
footprints={f['footprint']['uuid']:rows(f['source']) for f in json.loads((root.parent/'eda-prelayout/footprint-sources.json').read_text())['result']}
...
footprints.update({f['footprint']['uuid']:rows(f['source']) for f in read_result('local-footprints')})
...
for c in b['components']:
 ...
 if ref in expected and actual!=expected[ref]:net_errors.append(ref)
 if ref in plan['candidate_only_refs'] and any(actual.values()):net_errors.append(ref)
 def transform(u,v):
...
for (typ,pid),v in rb.items():
 if typ!='PAD':continue
 x,y=v['centerX']*.0254,v['centerY']*.0254
 w,h=v['defaultPad']['width']*.0254,v['defaultPad']['height']*.0254
 points=[[x-w/2,y-h/2],[x-w/2,y+h/2],[x+w/2,y+h/2],[x+w/2,y-h/2]]
 pads.append({'ref':v['num'],'pin':v['num'],'net':v['netName'],'bounds_mm':bbox(points),'corners':points})
 expected_pad=next(p for p in plan['flat_pads'] if p['number']==v['num'])
 assert v['netName']==expected_pad['net'] and abs(x-expected_pad['x'])<.002 and abs(y-expected_pad['y'])<.002
 assert v['hole'] is None and v['topSolderExpansion']>0 and v['topPasteExpansion'] < -max(w,h)/2/.0254
...
nfc_hits=[p['ref']+':'+p['pin'] for p in pads if overlap(p['bounds_mm'],[60,24,82,50])]
stack=sum(v.get('thickness',0) for (typ,pid),v in rb.items() if typ=='LAYER_PHYS')*.0254
with sqlite3.connect('file:eda/NFC-Business-Card-84x52.eprj2?mode=ro',uri=True) as db:database_check=db.execute('pragma quick_check').fetchone()[0]
...
report['drc']={group['name']:sum(part['count'] for part in group['list']) for group in b['drc']}
report['candidate_only_refs']=plan['candidate_only_refs']
report['standalone_pad_count']=len(plan['flat_pads'])
report['plan']=plan
report['copper']={t:sum(k[0]==t for k in rb) for t in ['LINE','VIA','POUR']}
