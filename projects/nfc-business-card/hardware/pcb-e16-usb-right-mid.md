...
仍有 23 个 GND 焊盘是孤岛，原因是没有可用的过孔落点（例如 U2、C7 的 0.4 mm 排距焊盘），或者该处覆铜被邻网切掉后过孔也接不到铜。**下一步**需要用铺铜的多边形数据（`pcb_PrimitivePoured.pourFills`）作为源，让布线器给这些焊盘各补一条短 GND 走线，而不是继续加过孔。
