...
3. **按键映射**：SW1（U1 pin 3 / P1.10）= 键 1，SW3（U1 pin 5 / P1.12）= 键 2，U1 pin 4（P1.11）是带上拉的备用输入、当前无开关。行为见下面第 5 节。

屏幕侧仍需固件确认的量（BUSY 极性、刷新时序、峰值电流）见[接口评估](gdeh0154e01-evaluation.md)。

## 5. 二级菜单与切换一致性

2026-09-21 确定基本交互：**两键完成全部基本操作。** 2026-09-23 硬件随之收敛为**两颗按键**：三颗并列时中心距只有 5.9 mm、本体只剩 0.7 mm 缝，因此删掉中间那颗，保留上下两颗（中心距 11.8 mm），外壳只开两个键孔；原第三键的备用输入仍留在主控上（带上拉），功能无需启用。以下“键 1 / 键 2”是逻辑角色，与 SW1/SW3、GPIO 及既有网络名的对应关系见[E16 设计记录](../hardware/pcb-e16-usb-right-mid.md#按键收敛为两颗2026-09-23)。
...
| 备用 | U1 pin 4 / P1.11，`KEY_NEXT_N` 上拉、无开关 | 不用；改版加回按键时可直接使用 |
| 第三键 | **已取消**（硬件只保留两颗）；备用 GPIO 保留，需要时可在改版中恢复 |
