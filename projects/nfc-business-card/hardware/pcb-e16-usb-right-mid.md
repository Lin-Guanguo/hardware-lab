...
落铜后**原生 DRC 与 Gerber 重导还没跑成**：EasyEDA 客户端此时对 `pcb_Drc.check()` 和制造包导出都超过 30 s 超时（轻量 API 正常），而这几步都需要客户端界面线程——当时 macOS 处于锁屏、窗口里很可能停着一个模态框。等解锁/关掉对话框后要做的三件事：

1. 跑原生 DRC，确认除 24 项固有 `Board Outline to SMD Pad` 外没有新增项（离线核验已经是 0 违规，这一步是复核覆铜与馈线的相互间隙）；
2. 重导 Gerber/BOM/CPL 并跑 `check-e16-manufacture.py`；
3. 把这一结果补回本节与[门槛文件](pcb-e14-manufacturing-gates.json)。
...
**离线校验结果**：线圈 + 两段馈线 + 顶层跨接段 + 2 个过孔，对板上**全部**既有铜线/焊盘/过孔逐点检查 —— **0 处违规**；环间距 0.4 mm 中心距 → 铜边距 **0.15 mm**（规则 0.102）；所有顶点离 J1 金属壳 **≥5 mm**（含铜边距）。也就是说几何已经可以直接落盘。
...
| 网表与电源布线 | **已完成** | 原理图四页 56 位号 = PCB 56 位号，**逐引脚比对 234 个引脚 0 处网名差异**（双向差集为空），四页 DRC 0；PCB **781** 段铜、242 焊盘、162 过孔、2 块 GND 覆铜，原生 DRC 间距 0 + 连接 0 + 孔到孔 0（仅 24 项同封装槽边告警）；电源网络最细 0.15 mm ≈ 600 mA @10 °C 温升 |
...
3. **NFC 天线形式与线圈。** 馈线与底层落点已落铜（见「NFC 天线馈线落铜」），剩下的是线圈本身与匹配电容：板上 PCB 线圈 or 外接 FPC/贴纸天线，等 PN532 桌面实验结论；可用线圈区约 17 × 21 mm（x 60–71.7、y 29–50），起点数据见 [NFC 馈线预研](#nfc-馈线预研2026-09-23)。
