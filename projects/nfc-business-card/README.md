...
- **CAD 进展（V4，当前）：**[E16 外壳样件 V4](enclosure/nfc-card-e16-enclosure-v4.FCStd) 跟随 L 形板框（左下角 33.5 × 15 mm 电池挖空）更新：电芯改坐在下壳底板上、不再压在 PCB 上，整机厚度从 5.0 降到 **4.5 mm**（0.4 底盖 + 0.3 板下间隙 + 0.8 PCB + 2.5 顶腔 + 0.5 上盖）。V3 的两块打印板也按 L 形生成，电池袋上下都缺板、加强筋悬空；V4 把打印板改成整卡轮廓减 USB 缺口，电池袋重新封闭（底板 201 mm³ + 上盖 251.25 mm³ 写进回归检查）。几何检查通过（上下壳各一实体、无壳体/参考件相交、连接器零接触）。上盖保留三条加强筋防止 0.5 mm 薄板塌陷；屏幕窗口按 GDEH0154E01 图纸压在有效区上、屏幕架空 1.51 mm 用垫片压紧。电芯最大外形、键帽、屏幕贴合与壁厚公差仍需实物验证。
