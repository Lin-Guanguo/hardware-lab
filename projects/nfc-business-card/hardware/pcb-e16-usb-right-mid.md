...
> **⚠️ 这条已在本节之后被实测否决，不要把「定制封装」当成结论。** 封装确实建出来了（`HL_NFC_COIL_E16`），但落板后 DRC 报 **12 项非基线**：本工具的规则要求**封装里的铜必须全部用焊盘做、且被铜连起来的焊盘必须同号**，因此「两端分别编号、中间用铜连起来」的元件做不出来。四条路的实测结论、以及原理图侧已预留的 NFC 接口，见[进度看板](../docs/progress.md#交接nfc-线圈唯一未落的板级项)。
...
7. **线圈落铜**：前提（放宽 `NO_WIRES`）已完成；**表示方式仍未定**——定制封装已被 DRC 否决，当前推荐「原理图短接符 + 线圈单网络」，或「单网络 + 白名单一处跨网络接触」。四条路的对照与所需动作见[进度看板](../docs/progress.md#交接nfc-线圈唯一未落的板级项)。
