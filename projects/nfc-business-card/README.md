...
更新于 **2026-09-22**。当前开工入口是 E14 工程内的 **E16 Right-Mid USB Study** 图页（独立 PCB 文档；E15 Clean Layout - New Battery 保留作底边 USB 回退）：沿 **84 × 52 mm（含外壳边缘）的定制 PCB/PCBA** 继续，屏幕位于左上，长条电池先按名义 **30 × 12 × 3 mm** 包体放在左下，NFC 净空移到右上，USB/充电/主控集中到右侧，USB 右侧中部作为首选研究方向，底边中部保留作回退。优先薄度和装配余量，≤5 mm 是目标，尚未实物验证；电池保护板、出线、胶带和鼓胀余量仍待确认。三键先保留，最终至少保证两键。
...
- **CAD 进展（V2，当前）：**[E16 外壳样件 V2](enclosure/nfc-card-e16-enclosure-v2.FCStd) 解决 V1 的两条待解约束：外壳改成**叠层式**（PCB 板框即整机外缘 84 × 52，两块壳各带 1.0 mm 内缩台阶，PCB 侧边外露），并按下拉 GCT 图纸把连接器前缘从估算的 x=88.45 修正为 **x=85.3**（外伸约 1.3 mm，整机长 85.3 mm < 名片标准 85.6 mm）；3 mm 电芯改放到 PCB 顶面（左下 x 3–33 无元件），叠层 0.4+0.3+0.8+3.0+0.5 = **5.0 mm** 成立。几何检查通过（上下壳各一实体、无壳体/参考件相交、连接器与两壳零接触）。代价：两块盖板只有 0.4/0.5 mm 厚，电芯最大外形、键帽、屏幕贴合与壁厚公差仍需实物验证。
