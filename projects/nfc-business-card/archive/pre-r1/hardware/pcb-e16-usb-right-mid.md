...
**2026-09-23 又落了两处优化，都由 DRC 看不见的规则集发现**：USB/ESD 区域补 **4 个地缝合过孔 + 5 条拉线**并重铺两块地覆铜（`ES-002` 从各 0 → 4 个过孔）；`C3`（充电器 BAT 引脚电容）从 USB 区搬回 **U2 旁 (49.60, 21.60)**（`DC-001` 从 24.49 mm 严重 → 2.04 mm 通过），顺带删掉它的 12 段绕行与 4 个孤立过孔。两处详见下文对应小节。
...
> **计数以机器为准。** 本文出现过多组图元计数，其中 **781 段铜 / 242 焊盘 / 162 过孔** 是 NFC 馈线落铜**之前**的过程值，**803 / 172** 是地缝合与 C3 修复**之前**的值。当前设计值从[门槛文件](pcb-e14-manufacturing-gates.json)的 `current_snapshot` 读（`copper_lines: 792`、`pad_count: 244`、`vias: 168`），并与[快照](e16-right-mid-snapshot.json)复核；两者冲突时以门槛文件与 `check-e14-gates.py` 为准（见[进度看板](../../../docs/progress.md)）。手工抄写的数字会漂，这是已发生的事实。
...
### ES-002 已修复：USB/ESD 区域补地缝合（2026-09-23 已落盘）

按上面的诊断落铜，**4 个 GND 过孔 + 5 条拉线**，并重铺两块地覆铜。计划由 [plan-e16-ground-stitch.py](../scripts/plan-e16-ground-stitch.py) 生成（[计划数据](e16-ground-stitch-plan.json)），落盘脚本 [apply-e16-ground-stitch.js](../scripts/apply-e16-ground-stitch.js) 由计划生成，避免手抄坐标。

| 目标 | 改动 | 尺寸 | 余量 |
| --- | --- | --- | --- |
| U5 的 GND 焊盘 (73.10, 16.60) | 0.40 mm 拉线直接进顶层覆铜 + 2 个过孔 | 0.61/0.305 mm（24/12 mil） | 拉线 0.329、过孔 0.268 / 0.230 mm |
| U6 的接地（经 R1 的 GND 焊盘，同一电节点） | 0.25 mm 拉线 + 2 个过孔 | 0.30/0.20 mm（11.8/7.8 mil） | 过孔 0.151 / 0.072 mm |

**U6 为什么从 R1 的焊盘下手**：U6 自己的焊盘被围死了——`USB_CC2` 走线横在它与顶层覆铜之间唯一的缺口上，实测能直线进入覆铜的 21 个方向里**连 0.15 mm 的拉线都不合法**，最近过孔位置只有 **0.0095 mm** 余量。R1 的 GND 焊盘与 U6 的 GND 焊盘重叠（同一节点），旁边 0.30/0.20 过孔有余量 0.151 mm 的位置，因此从那里接入。落点距 U6 的地焊盘 0.57 mm，在 ES-002 的 3 mm 判据内。**没有为了凑规则把过孔挤进 9.5 µm 的缝里。**

**验证（每一项都是实测）**：

| 项 | 改动前 | 改动后 |
| --- | --- | --- |
| `ES-002` U5 的 GND 焊盘 3 mm 内 GND 过孔 | 0 | **4** |
| `ES-002` U6 的 GND 焊盘 3 mm 内 GND 过孔 | 0 | **4** |
| U5 的地焊盘是否在底层覆铜内 | 否 | **是** |
| U6 到最近底层覆铜 | 2.318 mm | **0.256 mm** |
| 图元计数 | 798 线 / 168 过孔 | 803 / 172 |
| 覆铜（顶层 / 底层） | 29 / 14 区域，40.0% / 50.7% | 29 / 16 区域，40.0% / **51.0%** |

- **间距**：拿 git HEAD 的改动前快照当"既有板"，把新增的 5 线与 4 孔当"待布"跑 [check-proposed-route.py](../../../scripts/check-proposed-route.py)：**0 违规**，最小余量 `via_to_track` 0.224 mm（规则 0.152）、`hole_to_hole` 0.385 mm（规则 0.300）、`track_to_track` 0.249 mm（规则 0.102）。
- **持久化**：保存 → 关闭图页 → 重开，计数仍为 56 元件 / 244 焊盘 / 803 线 / 172 过孔。
- **连通性**：0 断网、0 死端。
- **原生 DRC 返回 `false`，但改动前也是 `false`**（门槛文件记录改动前就有 24 项 `Board Outline to SMD Pad` 槽边告警），所以这个布尔值对本次改动没有分辨力。本次改动的间距结论以上面那条独立 diff 校验为准。~~注意 DRC API 只返回布尔值，明细在 UI 面板里，没有读取接口~~ —— **这条论断是错的，已于同日推翻，见下文「DRC 明细其实可读」一节。**
- **制造包已失效**：`check-e16-manufacture.py` 报 `via drill has 168 hits, snapshot has 172 vias`。这是正确行为（产物随设计改动失效），放行前必须重导。门槛文件的 `current_snapshot` 与 `source.eda_project_sha256` 已同步刷新。

规划过程中修掉三个我自己的 bug，都值得记：**漏了"方案内部孔到孔"检查**（两个过孔相距 0.601 mm、孔间隙 0.297 mm < 0.300 却通过了）；**贪心选择在余量并列时不成立**（先选了同一行最近的一个，把其余合法位置全挡掉，改成穷举配对）；**把覆铜坐标又缩放了一次**（`pour_geometry` 返回的已是 mm，却按 mil 再转，算出"覆铜在 72 mm 外"）。

### C3 已搬回 U2 旁（2026-09-23 已落盘）

`C3` 是 BQ25186 的 BAT 引脚电容（2.2 µF）。E16 的 USB 排版把它归入"USB 小元件"搬到 (71.90, 20.60)，离 U2 的 BAT 焊盘 **24.49 mm**（E15 时约 4.7 mm），`DC-001` 判为严重。现搬回 **(49.60, 21.60) rotation 0**，落盘脚本 [apply-e16-c3-move.js](../scripts/apply-e16-c3-move.js)。

落点由搜索确定，判据是"每个焊盘只需避开**不同网络**的铜箔"：

| 项 | 值 |
| --- | --- |
| BAT 焊盘 (48.90, 21.60) 距 U2 的 BAT 引脚 (46.77, 18.91) | **3.431 mm**（原 24.49 mm） |
| 与最近同网络焊盘（电池落点 BP） | **2.04 mm** → `DC-001` 通过（阈值 5 mm 警告 / 8 mm 严重） |
| GND 焊盘 (50.30, 21.60) | **已在顶层覆铜内**（距离 0.000） |
| BAT 焊盘对其他网络的最小余量 | 0.2605 mm（规则 0.152） |
| GND 焊盘对其他网络的最小余量 | 0.2528 mm |
| 新接线 (48.90, 21.60)→(50.15, 22.85)，0.15 mm 宽 | 独立校验 **0 违规**，`track_to_track` 余量 0.6355 mm |

**顺带删掉的**：`BAT_PACK_TBD` 原来 18 段 / 45.66 mm 中，471–476 是 U2 的 BAT 引脚到电池落点的必需部分，**477–488 是绕到 C3 旧位置的 12 段迂回**，已删除；这些段上的 **4 个过孔**（51.00,18.90 / 57.50,8.60 / 59.70,6.40 / 65.00,7.80）只为那条绕行存在，一并删除。

这一步的验证链值得记：删除 12 段后连通性审计报 **`BAT_PACK_TBD` 有 1 处断网**——正是那 4 个孤立过孔（其中 3 个各自成组）。删净后 `ok=True`、0 死端。**这是连通性审计第一次拦住我自己的改动**，也正是它存在的意义。

图元计数 803/172 → **792/168**。保存 → 关闭 → 重开验证通过，重导快照与实况逐字段一致。制造包已按最终板重导：`check-e16-manufacture.py` **ok=true**，168 钻孔命中对应 168 过孔。

改动前后 `DC-001` 的对照（同一检查器分别跑变更前、变更后的快照）：`C3` critical → pass；`C21`、`C22` 是 EPD 电荷泵电容，**变更前后都是 warning**，与本次改动无关（它们不是 IC 供电去耦，`DC-001` 的判据针对去耦）。


## DRC 明细其实可读，以及由它抓出的三类缺陷（2026-09-23）

**我此前记录的一条"验证空白"是错的。** 本文与进度看板都写过"DRC API 只返回布尔值，明细在 UI 面板里，没有读取接口"。实际上 `PCB_Drc.check` 有两个重载：

```typescript
function check(strict, userInterface, includeVerboseError: false): Promise<boolean>;
function check(strict, userInterface, includeVerboseError: true):  Promise<Array<any>>;
```

`includeVerboseError: true` 直接返回违规明细（分组、错误类型、对象、实测值、规则要求）。我因为相信了那条自述，**连续两轮都没有去读它**，代价是：搬 C3 那次改动带着 6 项真实违规落盘，而我报告的是"0 违规"。

### 它抓出的三类缺陷

读出来的第一次，板上共 **30 项**违规；扣掉已知的 24 项 `Board Outline to SMD Pad` 槽边基线，剩下的 6 项全部来自我的改动：

| 错误 | 对象 | 实测 | 要求 |
| --- | --- | --- | --- |
| Clearance | `CHG_SDA` 走线 vs `C3_1` 焊盘 | **4.3 mil** | ≥ 6 mil |
| Clearance | `BAT_PACK_TBD` 走线 vs `C3_2` 焊盘 | **4.0 mil** | ≥ 6 mil |
| Connection | `C3_2` 焊盘 | 未连接 | — |
| Physical（×2） | 两个 GND 过孔 | 孔径 **7.8 mil** | 7.9 ~ 248 mil |
| Connection | 一个 GND 测试点 | 未连接 | — |

三类根因：

1. **我的临时焊盘间距脚本几何错了。** 它把矩形焊盘当成半径 `max(半宽, 半高)` 的圆。CHG_SDA 是一条 45° 斜线，矩形在斜向上的外延是**支撑函数** `|ux|·半宽 + |uy|·半高`，不是 `max(...)`；两者在 31.5×35.4 mil 的焊盘上差 0.151 mm ≈ **5.9 mil**，正好等于 DRC 报的 4.3 mil 与我算的 10.25 mil 之差。
2. **工具覆盖有缺口。** `check-proposed-route.py` 只查"待布走线 vs 既有焊盘"，**没有"被移动的元件焊盘 vs 既有走线"这一向**。我当初正因为缺这一向才去写临时脚本，也才把错误带了进来。已补 `pads` 模式。
3. **我的规划器缺"孔径下限"约束。** 地缝合规划器把 0.20 mm 孔径当成合法档位，而规则下限是 7.9 mil = 0.2007 mm——差 0.0007 mm。GND 网络上恰好只有我加的这 2 个过孔违规（同样 7.8 mil 的另外 28 个都在信号网络上，未被该规则约束）。

### 校准：检查器必须能复现 DRC 的结论

补完 `pads` 模式后，我用它复算**已知违规**的 C3 原位：

| | DRC | 我的检查器 |
| --- | --- | --- |
| BAT 焊盘 vs CHG_SDA | 4.3 mil | **4.34 mil** |
| GND 焊盘 vs BAT 走线 | 4.0 mil | **4.08 mil** |

误差 ≤ 0.04 mil。**这是"先验证工具再验证结果"在本项目最直接的一次落实**：不是相信自己的几何，而是让工具去复现一个已知的真值。

### 修复与复验

`apply-e16-c3-replace.js` 做了三件事：删掉上一次那条已随焊盘失效的拉线、把 C3 移到 **(48.60, 22.75) rotation 180**、把两个过孔的孔径从 7.8 改为 8.0 mil（直径 11.8 → 12.0，与另外 48 个 GND 过孔同档）。

这次编排的落点由 [plan-e16-c3-move.py](../scripts/plan-e16-c3-move.py) 搜索得到，判据是"每个焊盘只避开**不同网络**的铜箔 + 必须**真连**"：

- C3 的两个焊盘分别与电池落点焊盘 **BP 重叠 0.40 mm、与 BN 重叠 0.25 mm**，是实铜相接，不依赖覆铜热焊盘——上一次正是依赖热焊盘而 DRC 报未连接。
- 最差间距 `pad_to_track` **8.47 mil**（规则 6 mil）。
- 它同时基本就是 **E15 的原位**，是对设计意图的一次交叉验证。

**结果：原生 DRC 从 30 项降到 24 项，且 24 项全部是槽边基线，非基线违规 0 项。** 连通性 0 断网 0 死端；`DC-001` C3 **通过（0.82 mm）**；`ES-002` 仍为各 4 个过孔。

### 为什么 U2 的 BAT 引脚旁放不下

搜索覆盖了 U2 周围 x 45–51 / y 18.5–24.5 的全部四个朝向。**"不重叠、走短拉线"（拉线 0.05–2.6 mm）在这个区域内没有任何合法解**：U2 的 BAT 走线沿 y = 18.90 向东，其南侧 1.2 mm 处才有地铜箔（U2 自己的 GND 焊盘 (46.769, 20.104)、C2 的地焊盘与两个地过孔），而这条走廊被 `CHG_PG_N` 的过孔（约 x 47.6, y 20.3）与 `CHG_SDA` 占住，任何直线拉线都会撞上它们。所以唯一的解就是让焊盘与 BP/BN 实接。

### 规划器自己也被查出三个 bug

同一个"模型没排除它该排除的东西"的毛病：

1. 收集同网络焊盘时**没有按层过滤**，把其它层的焊盘算成了可接铜箔；
...
> **后续修正（E17）**：本地 API 文档中有 `sch_PrimitiveComponent.createShortCircuitFlag`，不需要 GUI 或自建短接符号。已在独立的 `Board1_3` 放置并落六圈线圈，网表、原生 DRC、制造导出和 CAD 干涉检查通过；详见 [E17 线圈研究](pcb-e17-coil-study.md)。以下保留此前试验的历史记录；其中“API 无法放短接符”及“100 pF + 15 pF 可作匹配起点”的结论已失效，后者还需按 Nordic 双侧电容拓扑重算并实测。
...
### 官方扩展怎么做的：它也不解决网络问题（2026-09-23）

用户提示了嘉立创官方的[线圈生成器扩展](https://github.com/easyeda/eext-coil-creator)（OSHWLab 官方发布，v1.3.1，MIT，313 次下载，专为"NFC 卡片、PCB 电机、无线充电"生成 PCB 线圈）。读了它的实现（`build/dist/index.html`），结论很直接：

```js
await eda.pcb_PrimitiveLine.create('GND', 1, x1, y1, x2, y2, trackWidth, false);
```

九处调用全部是这个形状。也就是说官方工具：**用走线（`pcb_PrimitiveLine`）画、单层、并把网络硬编码成 `'GND'`**——一个占位值。它不建焊盘、不建过孔、不用封装、也不用短接符。

**三条推论**：

1. **"线圈用走线画"是官方认可的做法**——我上一轮的方向没错，错的只是把它塞进封装。
2. **官方工具根本没有解决"两个网络"这件事**，它连尝试都没有，直接留一个 `GND` 占位，剩下的交给设计者自己改。README 的"待实现功能"里也没有相关条目。
3. 所以这个坑不是"我没找到正确接口"，而是**工具链本身就没解决**——与论坛那篇挂了五年的 net tie 功能请求互相印证。

### 制造侧规则：线圈板的表面工艺（一手来源）

用户另给的[嘉立创「线圈板的设计与表面工艺」](https://www.jlc.com/portal/q6i33776.html)是板厂对线圈板的加工要求，**这是本板目前还没纳入的约束**：

- 页面给出**线圈板最小线宽线隙表**（图片，需另行核对数值）。
- 表面工艺：**盖油墨**的线圈板可用喷锡 / 沉金 / OSP；**开窗不盖油墨**的线圈板**优先采用沉金或 OSP，尽量不要选喷锡**——开窗+喷锡会因工艺限制出现**粘连或厚薄不均，板厂不接受投诉**。

对本板的意义：NFC 线圈要不要开窗、开窗后选什么表面工艺，是**板厂明文规定的取舍**，不是可以随手定的；放行前必须把这两条落进制造参数。线圈的 0.25 mm 线宽 / 0.15 mm 线隙也要与上面那张最小线宽线隙表核对。


### 第三条路：线圈走单网络 + 一颗串联匹配元件（不需要短接符）

重读论坛那句话，它其实指出了一条**已经被证实能正常工作**的路：

> "At the moment **a 0 Ohms resistor or a series component as part of an impedance matching network works OK** because it has two separately numbered pads that are not connected in the Footprint by copper."

把它用到本板：**整条线圈铜箔挂在一个网络（如 `NFC1_TBD`）上，远端接一颗二脚串联元件，这颗元件的另一只脚落在 `NFC2_TBD`。**

- 线圈近端压在 NFC1 落点上 —— 同网络 ✓
- 线圈远端压在串联元件的 1 脚上 —— 也是 `NFC1_TBD` ✓
- 串联元件的 2 脚是 `NFC2_TBD`，与线圈铜箔**不相接** ✓
- 因此**没有任何跨网络接触，DRC 干净**，而且**不需要短接符、不需要封装、不需要改动原理图的短接语义**。

**这不是绕过规则，而正是 NFC 天线的标准拓扑**：差分天线本来就是"引脚 —匹配— 线圈 —匹配— 引脚"。放在单边做一颗串联匹配元件（电容或电感）就构成合法的串联匹配，而本板的匹配网络本来就还没定值（记录里写的是"匹配电容起点 100 pF + 15 pF，等实验拍板"）。

与另两条路的对比：

| 方案 | DRC | 网表 | 需要什么 |
| --- | --- | --- | --- |
| 原理图短接符 + 线圈单网络 | 干净 | 短接符声明合并 | 改原理图 |
| **线圈单网络 + 串联匹配元件** | **干净** | **真实元件，语义正确** | **只需定拓扑（值可后补）** |
| 官方扩展的做法（硬编码 `GND` 占位） | 需手工修 | 未处理 | —— |
...
### 下单侧的硬约束：线圈必须盖油（2026-09-23）

板厂对**线圈板**有专门规则，出自[下单前技术员必看](https://www.jlc.com/portal/server_guide_112.html)：

> **线圈板：线宽线距要求 0.254mm（极限 0.15mm：仅限线路盖油）。线圈被油墨盖住的，可以做喷锡或沉金工艺。线圈露铜的，只能做沉金工艺**

本板线圈是 **0.25 mm 线宽 / 0.15 mm 线隙**，两者都低于常规的 0.254 mm，**只落在"极限 0.15 mm"这一档**，而这一档**只允许线路盖油**。因此：

- **线圈区域必须保持被阻焊油墨盖住**——这是设计约束，不是可选的制造备注。
- 若要开窗露铜，线宽线距必须做到 **≥0.254 mm**，那是**几何重做**：环间距从 0.15 改到 0.254 会改变 6 圈螺旋的电感与 Q，`plan-nfc-coil.py` 算出的 1.12 µH / Q 139 需要重算并重新校验保留区。
- 之前那条"[开窗不盖油墨优先沉金/OSP](https://www.jlc.com/portal/q6i33776.html)"的建议针对的是**开窗**线圈，与本板无关：我们盖油，**线圈本身的表面处理无所谓**。

**下单流程本身不需要找客服**（查实于同一页面与板厂帮助文档）：

1. **设计信息在 Gerber 里**，板厂按文件做；能用文件表达的开窗、线宽、板框、孔都不需要口头沟通。
2. **表面处理/板厚/油墨颜色是下单页面选项**。注意[2026-08 公告](https://www.jlc.com/portal/factoryVisitDetails/62511)：**免费打样的喷锡券已全面取消、统一改为 OSP**，沉金券保留。OSP 的注意项（48 小时内焊接、最多 2 次回流+2 次波峰、不适合金手指与半孔板）本板都不涉及。
3. **关键机制是"生产稿确认"**：板厂把 Gerber 转成生产稿，**客户核对确认后**才投产（页面原话"尽量提供 Gerber 文件并下单确认生产稿检查"）。这比聊天可靠，是发现问题的正式环节。
4. **只有非常规要求才需要写明**，用**下单备注**；页面自己列了"请备注好"的情形（更小的插件孔、允许破孔、近似直角孔包边）。并明确"对于不符合上述表格数据的请自行调整好"——**设计不符规范要设计方改**。
5. 求助渠道：下单页顶部「技术咨询」（可搜历史问答/提问）、PCB 技术支持 QQ `3001741855`、官网「服务指引 → 人工服务」。

**本板按板厂规则逐项核对的结果**（数据取自快照与 `check-proposed-route.py`/`check-e16-manufacture.py` 的实测值）：

| 项 | 板厂要求 | 本板 |
| --- | --- | --- |
| **线圈线宽线距** | 0.254；**极限 0.15 仅限盖油** | 0.25/0.15 → **必须盖油** |
| 线圈表面处理 | 盖油→喷锡或沉金；露铜→只能沉金 | 盖油，线圈不受影响 |
| 免费打样表面处理 | 喷锡券取消→OSP；沉金券保留 | 无金手指/半孔 → OSP 可用 |
| 板框 | 唯一板框层且能正确输出 Gerber | 第 11 层 `BOARD_OUTLINE` ✓ |
| 锣边到铜 | **≥0.2 mm** | J1 焊盘 **0.2007 mm**，余量 0.0007 mm ⚠️ |
| 过孔孔到孔 | ≥0.2 mm | 最差 0.385 mm ✓ |
| 过孔孔径/塞油 | 过孔不受 0.5 mm 插件孔下限约束；单面开窗且孔径<0.4 mm 只能半塞（可能透光） | 过孔 0.305 mm → 半塞、可能透光，不影响功能 |
| 过孔开窗 | 双面开窗的过孔不能塞油 | 需在生产稿确认时核对本板过孔开窗设置 |

**J1 那条余量只有 0.0007 mm**，是放行前最需要在生产稿里盯一眼的项。
...
### 线圈落铜的前置与网表表示（2026-09-23，未落铜）

**已做的前提**：NFC 保留区（x 60–82, y 24–50）原本 `ruleType=[2,6,5,7]`，其中 **`5 = NO_WIRES`**（枚举见 [EPCB_PrimitiveRegionRuleType](../../../../../upstreams/easyeda-api-skill/references/enums/EPCB_PrimitiveRegionRuleType.md)）。线圈是走线，所以已放宽为 **`[2,6,7]`**：仍禁器件、铺铜与填充，只放行走线。改动是孤立的——元件/焊盘/走线/过孔/覆铜逐字段不变，只有 `regions` 的 ruleType 变了（提交 `6f11d90`）。

**发现的问题：原计划是短路，不是"替换铜箔"。** [e16-nfc-coil-plan.json](e16-nfc-coil-plan.json) 把整条螺旋标在新网络 `NFC_COIL` 上，从 NFC1 落点 (68.00, 23.00) 走到 NFC2 落点 (71.40, 23.00)；计划里的 `ignore_nets: ["NFC1_TBD","NFC2_TBD"]` 把这件事藏住了。去掉它重跑 [check-proposed-route.py](../../../scripts/check-proposed-route.py)：

```
ok = False | 136 项违规
  track_to_pad  gap = -0.125 mm  @ [68.0, 23.0, 'pad NFC1 NFC1_TBD']
  track_to_track gap = -0.200 mm @ [68.0, 23.0, 'NFC1_TBD']
```

原因不是几何而是表示：**线圈是两端串联元件**，两端分属 NFC1_TBD 与 NFC2_TBD。用"第三个网络"去碰这两个网络按定义就是短路；拆成两半分别命名，相接处又成为跨网络接触。**纯走线无法表示两端跨网络串联元件。**

**当时选定的做法：做成定制封装。** 螺旋作为封装的铜箔图形、两个焊盘接 NFC1_TBD / NFC2_TBD，schematic 侧加对应符号与器件，使逐引脚网表核对仍然有意义。另一条备选（合并 NFC1_TBD 与 NFC2_TBD 成一个网络）会使网表把线圈两端画成同一节点，从而让这两个网络上的逐引脚核对失去意义，未采用。

> **⚠️ 这条已在本节之后被实测否决，不要把「定制封装」当成结论。** 封装确实建出来了（`HL_NFC_COIL_E16`），但落板后 DRC 报 **12 项非基线**：本工具的规则要求**封装里的铜必须全部用焊盘做、且被铜连起来的焊盘必须同号**，因此「两端分别编号、中间用铜连起来」的元件做不出来。最终可行做法是 [E17 的原理图短接符与单网络走线](pcb-e17-coil-study.md)。
...
| 步骤 | 接口 | 说明 |
| --- | --- | --- |
| 1. 学封装 document source 格式 | `lib_Footprint.openInEditor(uuid, libUuid)` → 文档对象 → `getDocumentSource()` → 关闭 | `lib_Footprint.get()` **只返回元数据，没有 documentSource**；软件说明里已记过这条路 |
| 2. 建封装 | `lib_Footprint.create(libraryUuid, name, classification, description, otherProperty)` 然后 `lib_Footprint.updateDocumentSource(uuid, libUuid, source)` | source 里含焊盘与铜箔图形（螺旋） |
| 3. 建器件/符号 | `LIB_Device` + `LIB_Symbol` | 2 脚天线器件，关联上面的封装 |
| 4. 再放宽保留区 | `pcb_PrimitiveRegion.modify(id, {ruleType:[6,7]})` | 线圈包络 x 60.60–71.40、y 23.00–48.90 **大部分落在保留区内**，所以 `NO_COMPONENTS(2)` 也会挡住器件 |
| 5. 放置与连接 | 器件放到 (68.00–71.40, 23.00) 使两焊盘与落点重合 | 落点已由 NFC 馈线接好 |
| 6. 复验 | 重铺 → 保存关闭重开 → 重导 → DRC + 连通性 + 逐引脚网表 + 制造包 | 器件与封装一变，下游门禁全部失效 |

板上的库 uuid：`0819f05c4eef4c71ace90d822a990e87`（`lib_Footprint.search` 返回过）；`C0603` 的 uuid 是 `76d6c29a47804db487f8a92b2621751b`，可用它练习读取 source 格式。

### 待办（按依赖排序）

1. ~~给 USB/ESD 区域补地缝合孔~~ **已完成（2026-09-23）**：4 个过孔 + 5 条拉线已落盘并重铺，`ES-002` 从两处 0 个过孔变为各 4 个。
2. ~~处置 C3~~ **已完成（2026-09-23）**：搬回 (49.60, 21.60)，`DC-001` 从 21.90 mm 严重降为 2.04 mm 通过。
3. ~~重导制造包~~ **已完成（2026-09-23）**：按最终板 792 线 / 168 过孔重导，`ok=true`。
4. **把 `ground_reach` 修到可信**：目前报 2 个 GND 焊盘未到达覆铜，但都与手工验证的事实冲突（R13 的 GND 焊盘 0.050 mm 处就有 GND 过孔），标为 `trusted: false`、不参与门禁。修好后才能逐一验证 29+16 块覆铜区域与 GND 网络的连通性。
5. **补 `GP-001`（信号跨越覆铜缺口）**：**已实现但结论不可信**，见下条。
6. **`GP-001` 的走廊宽度需要校准**：已实现（`analyze-e16-pour.py`），按"采样点到最近参考铜箔的距离 ≤ 0.5 mm"判覆盖率。结果是 48 个信号网络里 **45 个低于 95%**，worst void 2.0–3.5 mm，集中在 EPD 电容列 (x 44–58, y 25–45) 与 NFC 禁布区。**这个结论不可信**：走廊宽度 0.5 mm 是我定的（上游只给 95% 阈值，没定义走廊），而该指标无法区分"真实平面开槽"与"平面被同层其它走线的间距穿孔"——密集布线本身就会把参考平面打满孔。标为参考项、不参与 `ok`；**要校准到一块已知有问题的板子才能用**。已知的、设计上就该存在的开槽是 NFC 禁布区与电池挖空。
6. ~~补原生 DRC 明细的可读路径~~ **已完成**：`pcb_Drc.check(_, _, true)` 返回明细数组，见「DRC 明细其实可读」。
7. ~~线圈落铜~~ **E17 候选已完成**：原理图短接符 + 单网络走线，原生 DRC 新增项 0；匹配电容及 RF 实测仍未完成，见 [E17 记录](pcb-e17-coil-study.md)。
8. **`C21` / `C22`（EPD 电荷泵电容）的 `DC-001` 警告**：变更前就存在（5.12 / 6.30 mm）。属电源变换电容而非 IC 去耦，`DC-001` 的判据不直接适用，但连接电感对开关回路 EMI 同样有意义，值得单独评估。
...
> **后续更正（2026-09-23）**：以下是 E17/E18 小线圈的历史推导。把 5 mm 经验余量分别施加到 X 与 Y 轴，过早限制了右边界；它不是厂商给出的通用金属净空值。按 J1 实际包络重算并验证后的 E19 五圈放大候选见 [线圈面积与嘉立创工艺复核](nfc-coil-area-process-review.md)。

把上一节"待定"的部分先做成可直接落地的方案。**当时把可用区收窄**：保留区 x 60–82 / y 24–50，但离 J1 金属壳（x 76.704–83.2、y 10.22–21.77）要留 5 mm，于是当时取线圈右边界 x≈71.7；再各留 0.5 mm 余量后，外圈取 **x 60.60–71.00、y 27.60–48.90（10.4 × 21.3 mm）**——比早先估的 17 × 21 小一圈，电感也跟着小一档，所以下面的数字是当时的估算结果。
...
**选 6 圈**（1.12 µH、Q 139，均为模型估算）。`123 pF` 是线圈在 13.56 MHz 的**差分等效谐振电容**，此前给出的“100 pF + 15 pF 两个并联位”未按 nRF52840 官方要求的两侧各一只匹配电容换算，**不能作为贴装值或下单依据**；修正与待测项见 [E17 记录](pcb-e17-coil-study.md#rf-必须修正的地方)。
...
注意事项：`getNetlistFile()` 在本机返回 null，网表仍走原理图导出的 enet；导出文件由 `sys_FileSystem.saveFile()` 写到 `~/Downloads` 的隐藏临时名（`.cn.lceda.pro.*`），需要按大小认领。**放行下单仍受 [E14 制造门槛](pcb-e14-manufacturing-gates.json) 约束**——电池最大包络和外壳公差未确认前只做验证导出。
...
**历史估算，不能直接落铜**：下表把 13.56 MHz 所需的差分等效电容 `C = 1/((2πf)²L)` 误当作单侧外部电容，没有按 Nordic 要求的 NFC1/NFC2 两侧各一只匹配电容换算。数值只保留作早期电感比较；正确拓扑和待测事项见 [E17 RF 修正](pcb-e17-coil-study.md#rf-必须修正的地方)。
...
3. **NFC 天线与匹配。** E17 独立候选已经把六圈 PCB 线圈落铜并通过网表、DRC、制造与 CAD 检查；双侧匹配电容焊位和 RF 实测仍待完成，见 [E17 记录](pcb-e17-coil-study.md)。本节此前的“线圈尚未落铜”和“100 pF + 15 pF”属于历史状态。
...
6. **制造包放行。** 上述确认后导出 Gerber/BOM/CPL 作为下单文件；当前导出只作验证，`ordered_next_actions` 见[门槛文件](pcb-e14-manufacturing-gates.json)。

**2026-09-23 二次审查修正**：上文“制造包检查通过”只针对层存在、L 形轮廓的顶点/外包络、钻孔数与 BOM/CPL 数量。实际 `.GKO` 还带一条 J1 封装开放辅助轮廓，`.GME` 带完整 84 × 52 mm 矩形。`check-e16-manufacture.py` 补上“唯一闭合轮廓、无冲突矩形”后对旧导出返回 `ok=false`；E18/E19 同样受影响。源板的名义板框与 CAD 检查仍成立，但**这些旧 Gerber 压缩包不能下单**。见 [整板复核](pcb-e19-independent-review.md)。
