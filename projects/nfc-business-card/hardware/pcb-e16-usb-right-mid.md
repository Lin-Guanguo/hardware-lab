...
更新于 **2026-09-22**。用户确认 USB 放到**右边缘中部**更符合卡片形态；本方案已**写入 E16 图页**并通过保存、关闭、重开验证。板框为 84 × 52 mm，底边恢复直边，右边缘中部开 **9.24 × 6.5 mm** 缺口，J1 插口朝 **+x**；U1 转 180° 让模块天线端朝下板边。**除 GND 外的 45 条网络已全部布通**，原生 DRC 只剩 1 项既有间距错误、12 项同封装槽边告警和 54 项 GND 未连接；GND 需靠覆铜解决，接口待客户端 GUI 操作（见下）。
...
| 覆铜 API | `pcb_PrimitivePour.create()` 在本机 3.2.203 上六种入参全部被拒（`无法创建覆铜边框图元`），GND 只能走客户端 GUI |

### 当前 DRC（保存、关闭、重开之后）

| 类别 | 数量 | 处理 |
| --- | ---: | --- |
| 普通间距错误 | 1 | 既有 BAT_PACK 测试点与 R6_2 焊盘 0.05 mm，非本轮引入 |
...
1. 在 E16 图页选 **放置 → 覆铜**，图层选**顶层**；
2. 沿板框拉一个覆盖整板的矩形（缺口会自动避让）；
3. 属性面板把网络设为 **GND**，铺铜方式保持默认，确定；
4. 重复一次，图层换**底层**；
5. 保存。

之后我用 API 复核 DRC：预期 GND 的 54 项未连接大幅下降，同时要确认没有新增间距错误。剩下 1 + 12 项属于既有基准问题，不随覆铜变化。
