...
c=json.loads((ROOT/'hardware/r1-design.json').read_text())
...
for i,(x,y) in enumerate(c['swd']['pad_centers_mm'],1):
    ...
    assert error<.002
    probe=Part.makeCylinder(.99,1.13,App.Vector(x,y,-.01))
...
report={'status':'NOMINAL_CAD_AND_PRINT_MESH_PASS','total_height_mm':5.8,'intersection_count':0,
        ...
        'limits':['Nominal geometry only; resin and assembly tolerances unqualified.','Battery, flex, USB plug, key feel and adhesive retention require sample assembly.']}
