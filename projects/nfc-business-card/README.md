...
- **全板布线已完成（2026-09-22 晚）：除 GND 外的 45 条网络全部连通**，当前板面 652 段铜线、112 个过孔。USB 七网之后，用新增的 [离线迷宫布线器](scripts/pcb-maze-router.py)逐个收口 12 条被自动布线放死的连接（五条 EPD 控制线、EPD_DC/EPD_BUSY 重排、充电器五条配置线、SYS、VDD_3V3、BAT_PACK），拆掉 18 段挡路旧线、补 246 段新线。保存、关闭、重开后原生 DRC：**普通间距错误 1 项（既有 BAT_PACK 测试点冲突）、同封装槽边告警 12 项（既有 J1 沉板槽）、连接错误 54 项且全部是 GND**。E7/E11 时代"两层做不出 USB 数据扇出"的结论已被推翻。
- **GND 覆铜已铺：**`pcb_PrimitivePour.create()` 只有在多边形**显式闭合**时可用（不闭合时一律报"无法创建覆铜边框图元"）。已落 `GND_TOP` / `GND_BOT` 两块整板覆铜并 rebuild，GND 未连接从 54 降到 32；再给 26 个被切碎的 GND 焊盘补了缝合过孔，进一步降到 **23**。剩余 23 个孤岛要用铺铜多边形做源，让布线器补短走线，详见 [E16 记录](hardware/pcb-e16-usb-right-mid.md#gnd-覆铜)。
...
- 下一步按[E16 右侧中部 USB 布局](hardware/pcb-e16-usb-right-mid.md)继续：**清掉剩余 23 个 GND 孤岛**（用铺铜多边形当源补短走线）→ 复核地回流 → 核对 NFC 预留区的禁布约束与馈线方案 → 刷新审查图并把外壳样件推进到含卡扣与公差、与冻结板框一致的版本 → 最后才是制造导出与下单。到货后仍需验证 FPC、刷新/功耗、电池与 NTC。
