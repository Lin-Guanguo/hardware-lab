"""Generate the E16 enclosure sample V2 for the right-mid USB layout.
...
MODEL = OUT / "nfc-card-e16-enclosure-v2.FCStd"
STEP = OUT / "nfc-card-e16-enclosure-v2.step"
BOTTOM_STL = OUT / "nfc-card-e16-bottom-v2.stl"
TOP_STL = OUT / "nfc-card-e16-top-v2.stl"
REPORT = OUT / "nfc-card-e16-enclosure-v2-report.json"
...
CAV_T = 3.0                       # top cavity: 301230 cell sits on the PCB top
...
doc = App.newDocument("NfcCardE16EnclosureV2")
doc.Label = "NFC 名片 · E16 右侧中部 USB 外壳协调样件 V2"
doc.Comment = "外壳外缘 = PCB 板框（叠层式）；连接器前缘按原厂图纸修正到 x=85.3。"

params = doc.addObject("App::FeaturePython", "EnclosureParameters")
params.Label = "E16 外壳 V2 参数与证据"
...
# --- bottom shell -----------------------------------------------------------
bottom_shape = outline_prism(0.0, PLATE_B)
...
bottom = add_feature(doc, "BottomShell", "下壳 · 右边缘 USB 让位", bottom_shape, "print_candidate", "#8FA9C1")
...
report = {
    "status": "E16_ENCLOSURE_V2_SAMPLE_NOT_PRODUCTION_RELEASE",
    "outer_mm": [W, H, round(OUTER_H, 3)],
    "size_basis": {
        "structure": "laminate: PCB outline is the card outline, shells carry a 1.0 mm inboard ledge",
        "card_outline_mm": [W, H],
        "pcb_outline_mm": [W, H],
    ...
    "geometry_checks": {
        ...
        "findings": findings,
        "valid": not invalid and not shell_intersections and all(v <= 1e-6 for v in contact.values()),
    },
    "limitations": [
