...
注意 E16 仍是独立 PCB 文档、没有和 `Board1_1` 关联；本页的网表来自当初的导入。位号与网络一致，但若要让 E16 成为正式生产图页，需要在客户端把它并入 `Board1_1`（API 只能新建 PCB，不能把已有 PCB 挂到板下）。
...
| 禁布区内铺铜采样（逐点落在填充多边形内判定） | 顶层 **0/49**、底层 **0/49** 点有铜 |
| 上方 1 mm 对照带 | 底层 **49/49**、顶层 5/49（顶层该处本来就有模块焊盘和净空） |
| 板中对照带 | 顶层 35/49、底层 34/49（证明采样方法本身能检出铜） |
...
| 门槛 | E16 状态 | 证据 |
| --- | --- | --- |
| 电池交付包络 | **待输入** | 需要电芯实物最大外形、保护板、出线折弯、胶带与鼓胀余量；外壳把电芯放在 PCB 顶面，顶腔 3.0 mm |
...
| 网表与电源布线 | **已完成** | 原理图四页 56 位号 = PCB 56 位号（双向差集为空），四页 DRC 0；PCB 原生 DRC 间距 0 + 连接 0（仅 12 项同封装槽边告警）；电源网络最细 0.15 mm ≈ 600 mA @10 °C 温升 |
| NFC RF 与铜箔净空 | **预留区已验证，天线待定** | 22 × 26 mm 预留区（x 60–82、y 24–50）实测**双面零铜**（无走线、无过孔、无焊盘，铺铜被禁布区裁掉），符合线圈下方不留金属的约束；线圈形式等 PN532 桌面实验结论 |
| CAD 可打印外壳 | **样件完成，待实物** | [外壳 V2](../enclosure/nfc-card-e16-enclosure-v2.FCStd) 几何检查通过（上下壳各一实体、无相交、连接器零接触）；屏幕窗口按 GDEH0154E01 图纸压在有效区上；屏幕需架空 1.51 mm 并用垫片压紧；壁厚、粘接、键帽、公差待样件 |
