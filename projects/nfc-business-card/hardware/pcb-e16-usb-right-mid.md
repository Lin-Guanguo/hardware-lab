...
**顺带发现（与本次改动无关，早于它存在）**：当前规则集（`JLCPCB Capability(Multiple Layers Board)`）要求孔到孔 ≥0.3 mm，而板上一对 GND 过孔（`32.80, 28.00` 与 `33.10, 27.60`）的孔边距是 **0.2968 mm**，差 3 µm，会报 1 项 `Hole to Hole`。这一对在已提交快照里就是这个数值，属既有几何；是否要把其中一颗挪 0.1 mm 由后续决定。
