"""Generate the E16 enclosure sample V7 for the right-mid USB layout.

V7 fixes a defect found by probing the saved V3 solid: V3 built both printed
...
MODEL = OUT / "nfc-card-e16-enclosure-v7.FCStd"
STEP = OUT / "nfc-card-e16-enclosure-v7.step"
BOTTOM_STL = OUT / "nfc-card-e16-bottom-v7.stl"
TOP_STL = OUT / "nfc-card-e16-top-v7.stl"
CAPS_STL = OUT / "nfc-card-e16-keycaps-v7.stl"
REPORT = OUT / "nfc-card-e16-enclosure-v7-report.json"
...
WINDOW_MARGIN = 0.4
# three SKQGABE010 switches: two in the west column, one offset east (PCB plan 2026-09-23)
KEYS = ((38.1, 3.30), (38.1, 15.10), (46.3, 8.80))
...
doc = App.newDocument("NfcCardE16EnclosureV7")
doc.Label = "NFC 名片 · E16 右侧中部 USB 外壳协调样件 V7"
doc.Comment = "外壳外缘 = PCB 板框（叠层式）；连接器前缘按原厂图纸修正到 x=85.3。"

params = doc.addObject("App::FeaturePython", "EnclosureParameters")
params.Label = "E16 外壳 V7 参数与证据"
...
params.Status = "E16_ENCLOSURE_V7_SAMPLE_NOT_PRODUCTION_RELEASE"
...
for index, (kx, ky) in enumerate(KEYS, 1):
    references[f"Button{index}Reference"] = (
        f"SW{index} 参考 · ({kx:.2f}, {ky:.2f})",
        Part.makeBox(5.2, 5.2, 1.5, App.Vector(kx - 2.6, ky - 2.6, SPLIT_Z)), "#E4B28F", False)
...
for (kx, ky) in KEYS:
    top_shape = top_shape.cut(Part.makeCylinder(2.3, PLATE_T + 0.4,
                                                App.Vector(kx, ky, TOP_PLATE_Z0 - 0.2)))
top_shape = top_shape.removeSplitter()
...
caps = []
for index, (kx, ky) in enumerate(KEYS, 1):
    disc = Part.makeCylinder(CAP_DISC_R, PLATE_T, App.Vector(kx, ky, TOP_PLATE_Z0))
    boss = Part.makeCylinder(CAP_BOSS_R, TOP_PLATE_Z0 - CAP_BOSS_BOTTOM_Z,
                             App.Vector(kx, ky, CAP_BOSS_BOTTOM_Z))
...
pressed = []
for cap, (kx, ky) in zip(caps, KEYS):
    ...
    body = Part.makeBox(5.2, 5.2, 1.25, App.Vector(kx - 2.6, ky - 2.6, SPLIT_Z))
    pressed.append({
        "cap": cap.Name,
...
report = {
    "status": "E16_ENCLOSURE_V7_SAMPLE_NOT_PRODUCTION_RELEASE",
    ...
    "button_holes_mm": [[round(kx, 2), round(ky, 2)] for kx, ky in KEYS],
    "geometry_checks": {
