...
* Gerber zip carries every layer the fabricator needs. Exactly one closed path
  must describe the L-shaped board with the USB notch, with no conflicting
  full-size mechanical outline. Via drill count must match the snapshot.
...
def gerber_draw_paths(gerber_text: str) -> list[list[tuple[float, float]]]:
    """Collect the plotted XY paths, split at Gerber move commands."""
    match = re.search(r"%FSLAX(\d)(\d)Y(\d)(\d)\*%", gerber_text)
    if not match:
        return []
    scale = 10 ** int(match.group(2))
    paths: list[list[tuple[float, float]]] = []
    for line in gerber_text.splitlines():
        command = re.search(r"X(-?\d+)Y(-?\d+)D0([123])\*", line)
        if not command:
            continue
        point = (int(command.group(1)) / scale, int(command.group(2)) / scale)
        if command.group(3) == "2":
            paths.append([point])
        elif command.group(3) == "1" and paths:
...
def main() -> int:
    ...
    refs, via_count = snapshot_data(snapshot)
    ...
    with zipfile.ZipFile(files["gerber"]) as archive:
        ...
        if outline_name:
            outline_text = archive.read(outline_name).decode("utf-8", "ignore")
            points = gerber_points(outline_text)
            paths = gerber_draw_paths(outline_text)
            report["outline_draw_paths"] = len(paths)
            if len(paths) != 1 or paths[0][0] != paths[0][-1]:
                problem(f"outline layer has {len(paths)} drawn paths instead of one closed contour")
        ...
        for name in names:
            if not re.search(r"\.GM(?:E|L|\d*)$", name):
                continue
            for path in gerber_draw_paths(archive.read(name).decode("utf-8", "ignore")):
                xs, ys = zip(*path)
                if (path[0] == path[-1] and min(xs) < 0.01 and min(ys) < 0.01
                        and abs(max(xs) - 84) < 0.01 and abs(max(ys) - 52) < 0.01):
                    problem(f"{name} contains a second complete 84x52 mm outline")
