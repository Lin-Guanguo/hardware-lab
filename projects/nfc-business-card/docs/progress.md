...
**一句话状态（2026-09-23）**：PCB 与 CAD 设计已收口到"可以打样验证"——E16 板（三键、L 形板框、右边缘 USB、NFC 馈线与底层落点）的原生 DRC、逐引脚网表、连通性、制造包全部通过；外壳 V7 的几何、真实元件干涉与 STL 网格检查通过并可直接打印。**板上只剩 NFC 线圈本身没落铜**（前置条件已办、表示方式已定为定制封装），其余都在等外部输入。2026-09-23 复验：原生 DRC 24 项槽边基线、非基线 0 项。

> **2026-09-23 方法论检查：两项都已修复。** 用 DRC 看不见的规则集（[规则索引](../../../docs/pcb-design-rules.md)、[pcb-methodology skill](../../../.agents/skills/pcb-methodology/SKILL.md)）离线扫出两处缺陷，都已落盘并复验：**USB/ESD 区域几乎没有地缝合**（`ES-002` 各 0 → 4 个过孔）与 **C3（充电器 BAT 引脚电容）离它要服务的引脚 24.49 mm**（`DC-001` 从 CRITICAL → 通过，2.04 mm）。两者原本都躲过了原生 DRC、逐引脚网表、连通性与制造包四项检查。制造包已按新板重导并通过。细节见 [E16 记录 · 方法论检查](../hardware/pcb-e16-usb-right-mid.md#方法论检查emc--信号完整性2026-09-23)。
...
- [x] **线圈现在就画**（2026-09-23 用户确认）。前提已完成：NFC 保留区的 `NO_WIRES` 已放宽（`[2,6,5,7]` → `[2,6,7]`，提交 `6f11d90`）。**但原计划画不出来**：它把整条螺旋标在新网络 `NFC_COIL` 上，铜箔直接压在 NFC1_TBD 的焊盘上（-0.125 mm，136 项违规）——计划的 `ignore_nets` 把这个短路藏住了。已定做法：做成**定制封装**（螺旋为封装铜箔图形 + 2 焊盘接 NFC1_TBD / NFC2_TBD，schematic 加对应器件），使逐引脚网表核对保持有意义。步骤与 API 见 [E16 记录](../hardware/pcb-e16-usb-right-mid.md#线圈落铜的前置与网表表示2026-09-23未落铜)。
...
- [x] **C3 处置**（2026-09-23 已落盘）：搬回 (49.60, 21.60)，`DC-001` 从 21.90 mm 严重降为 2.04 mm 通过。顺带删掉它那段 12 段绕行与 4 个只为绕行存在的孤立过孔；删除后连通性一度报 1 个断网，正是这 4 个过孔造成的，删净后 `ok=True`。
- [x] **重导制造包**（2026-09-23）：按最终板（792 线 / 168 过孔）重导并核对，`check-e16-manufacture.py` **ok=true**，168 钻孔命中对应 168 过孔。
- [x] **原生 DRC 明细可读**（2026-09-23）。`pcb_Drc.check(_, _, true)` 返回违规明细数组——**我此前"DRC 只有布尔值"的论断是错的**，代价是搬 C3 时带着 6 项真实违规落盘却报告"0 违规"。读出后全部修复，现在 DRC 为 24 项槽边基线、非基线 0 项。
