...
function safeJson(value) {
  ...
  try {
    ...
    return JSON.stringify({ success: false, error: 'result not serializable: ' + (err && err.message) });
  }
}
process.on('uncaughtException', (err) => {
  console.error('[guard] uncaughtException:', err && err.message);
});
process.on('unhandledRejection', (err) => {
  console.error('[guard] unhandledRejection:', err && err.message);
});
    ...
    ws.on('message', async (raw) => {
      try {
        ...
        if (msg.type === 'execute') {
          ...
          ws.send(safeJson({ type: 'pong', id: msg.id, timestamp: Date.now() }));
        }
      } catch (err) {
