...
采用含壳 **84 × 52 mm 的定制 PCB/PCBA**，厚度 ≤5 mm 仍待验证。最新工程与未完成项见[电路与试布线记录](../hardware/pcb-e6-84x52.md)。成品小板和飞线保留作桌面验证与历史备选。

首烧优先委托嘉立创完成，日常通过 USB-C 升级；暂不采购 SWD 烧录器，但保留 SWD 焊盘作为工厂烧录和故障恢复入口。**注意：E16 板上三个 SWD 测试焊盘（TP3/TP4/TP5，x 10/12.5/15、y 46）在顶层且在屏幕包络内，装壳后不可直接接触**——裸板与半成品可用，装壳后的救援只能走 USB-C；若要保留装壳后的 SWD 通路，需要在底层镜像焊盘并在下壳开探针孔（见 [E16 记录](../hardware/pcb-e16-usb-right-mid.md)的「SWD 救援入口的位置限制」）。此处是交付目标，尚无工厂承接确认，项目也尚未生成可交付固件。
