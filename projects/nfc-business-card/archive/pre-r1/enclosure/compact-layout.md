...
这不是把器件整体缩放：两种新布局保留屏幕、电池、主控、USB、FPC 座及按键的原尺寸和 Z 高度。83 × 51 也生成并检查了独立模型，临时结果在 `artifacts/archive/compact-layout/83x51-final/`，可用同一生成命令复现；当前只归档推荐版的 `.FCStd`。
...
- **离线封装：通过本次范围。** 对此前 Bottom-USB 的 27 个元件、145 个焊盘作建议平移，所有焊盘角点位于建议板框内，未发现跨器件焊盘/本体包围框重叠或占用 NFC。只读快照与计算记录在 `artifacts/archive/compact-layout/`；结果及建议坐标已归档至 `compact-layout-review.json`。
