...
1. ~~给 USB/ESD 区域补地缝合孔~~ **已完成（2026-09-23）**：4 个过孔 + 5 条拉线已落盘并重铺，`ES-002` 从两处 0 个过孔变为各 4 个。详见上文。
2. **处置 C3**（需客户端）：充电器的 BAT 引脚电容搬回 U2 旁。会牵动 `BAT_PACK_TBD`（现 18 段、45.66 mm）约 20 mm 走线的重布。
3. **重导制造包**：当前磁盘上的钻孔文件是改动前的 168 孔，板子已是 172。放在 C3 之后再导，避免白做一轮。
